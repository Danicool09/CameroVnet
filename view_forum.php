...
<div class="container-field">
	<div class="col-lg-12">
		<div class="card">
			<div class="card-body">
                ...
                <span class="float-right mr-4"><small><i><?php echo date('M d, Y h:i A',strtotime($date_created)) ?></i></small></span>
                 <span class="float-right mr-4 text-primary"><small><i>Creado por:: <?php echo ucwords($name) ?></i></small></span>
                 <div class="col-md-8">
					<h4><b><?php echo $title ?></b></h4>
				</div>
				<?php if(count($tags) > 0): ?>
				<div>
					<span class="badge badge-default"><i class="fa fa-tags"></i> Categorías: </span>
				...
				<hr>
				<div class="w-100">
				<span class="badge badge-secondary text-white"><?php echo number_format($view) ?> Visitas</span>
				<span class="badge badge-primary text-white ml-2"><i class="fa fa-comments"></i> <?php echo number_format(count($com_arr)) ?> Comentarios</span>
				</div>
		...
		<div class="card">
    		...
    		<div class="col-lg-12">
    			<div class="row">
    				<h3><b> <i class="fa fa-comments"></i> Comentarios</b></h3>
    			</div>
    			...
    			<div class="form-group comment">
                    ...
                    <?php endif; ?>
	                <span class="float-right mr-4"><small><i>Creado el: <?php echo date('M d, Y h:i A',strtotime($row['date_created'])) ?></i></small></span>
    				...
    				<div>
    					...
    					<span class="text-primary ml-4"><?php echo isset($rep_arr[$row['id']]) ? count($rep_arr[$row['id']]).(count($rep_arr[$row['id']]) > 1? ' Respuestas':' Responde') : '' ?></span>
    					...
    					<div class="col-lg-8 offset-lg-2 replies">
    						...
    						<div class="form-group ty-compact-list">
			                    ...
			                    <?php endif; ?>
				                <span class="float-right mr-4"><small><i>Creado el: <?php echo date('M d, Y h:i A',strtotime($rep['date_created'])) ?></i></small></span>
...
<script>
	...
	$('.edit_topic').click(function(){
		uni_modal("Editar Discusion","manage_topic.php?id="+$(this).attr('data-id'),'mid-large')
		
	})
	$('.edit_comment').click(function(){
		uni_modal("Editar comentario","manage_comment.php?id="+$(this).attr('data-id'),'mid-large')
		
	})
	$('.edit_reply').click(function(){
		uni_modal("Editar respuesta","manage_reply.php?id="+$(this).attr('data-id'),'mid-large')
	...
	$('.delete_topic').click(function(){
		_conf("¿Estas seguro de borrar esta discusión?","delete_topic",[$(this).attr('data-id')],'mid-large')
	})
	function delete_topic($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Información borrada exitosamente",'success')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	function submit_reply_func(){
		$('#manage-reply').submit(function(e){
			...
			$.ajax({
				...
				success:function(resp){
					if(resp == 1){
						alert_toast("Información guardada exitosamente.",'success')
						setTimeout(function(){
							location.reload()
						},1000)
					}
    ...
    $('.delete_comment').click(function(){
        _conf("¿Estas seguro de que quieres borrar este comentario?","delete_comment",[$(this).attr('data-id')],'mid-large')
    })
    ...
    $('.delete_reply').click(function(){
        _conf("¿Estas seguro de que quieres borrar esta respuesta?","delete_reply",[$(this).attr('data-id')],'mid-large')
    })
