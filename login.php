...
<style>
	...
	#login-left{
		...
		display: flex;
		align-items: center;
		background: url(assets/img/fotoLogin.JPG);
		...
		$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#login-form').prepend('<div class="alert alert-danger">El Nickname o la contraseña son incorrectos, intentelo de nuevo</div>')
					$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
				}
			}
		})
